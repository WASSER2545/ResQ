...
df = pd.read_csv('./dataset/eval_num.csv')
...
num_model = LinearRegression(fit_intercept=True)
num_model.fit(X_train, y_train)

y_pred = num_model.predict(X_test)

print("coef:", num_model.coef_)
print("intercept:", num_model.intercept_)
...
def num_predict_from_scalar(num_eval, card):
    return num_model.predict(np.array([[card * num_eval]]))
